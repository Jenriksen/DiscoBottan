...
namespace DiscoBottan.Commands
{
    ...
    public class ExampleGrouppedCommands
    {
        ...
        public async Task ChangeNickname(CommandContext ctx, [Description("Member to change the nickname for.")]
            DiscordMember member, [RemainingText, Description("The nivkname to give that user.")]
            string newNickname)
        {
            await ctx.TriggerTypingAsync();
